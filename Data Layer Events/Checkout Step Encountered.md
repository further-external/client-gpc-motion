...
This event is set during the checkout process when a visitor reaches an intermediary step in the process (Payment Details, Shipping, Order Review, etc).
...
| **checkoutStep** | string | Describes a discrete step in the checkout flow | Checkout Start, Contact & Order Information, Shipping Details, Continue as Guest (for users not signed in and chose to check out as guest) |
