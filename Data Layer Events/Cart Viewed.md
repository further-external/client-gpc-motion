...
This event fires when a visitor views the cart. It is part of the page load sequence and must be pushed between the **Page Load Started** and **Page Load Completed** events.
...
| **productId** | string | Unique Identifier of a product or offering. Must match the format of back-end systems. | 15,565,588,987,764,400 |
...
| **productSearchPhrase** | string | Describes the search keyword exactly as entered by the user. | red lobster, red lboster, red lbstr, Zip code |
...
| **quoteRequired** | string | Set with a value of "TRUE" or "FALSE" for each item when a cart is saved, saved to a list, or products are moved to a cart. | TRUE -OR- FALSE |
| **sellingPrice** | string | This should be added as the price for the product including the discount amount. | 97.88, 805.09 |
| **sku** | string | Stock Keeping Unit (SKU) Unique Identifier of specific item. | 34567890, 4567890, 00155-large-cornflower |
...
| **thresholdLocation** | string | Location where threshold change occurred | "pdp","plp","cart", "best sellers", "recommended items" |
